...
import { computeFileHash } from "@/lib/hash"; // Ensure this path is correct
...
export default function Home() {
    ...
    const verifyDocument = async () => {
        ...
        if (!hashToVerify || !provider) return; // Need provider to verify even if read-only

        setLoading(true);
        setVerifyResult(null);
        try {
            // Can use provider for read-only calls
        ...
        <main style={{ padding: "2rem", fontFamily: "sans-serif", maxWidth: "800px", margin: "0 auto" }}>
            ...
            <section>
                ...
                {hash && (
                    <div style={{ marginTop: "1rem" }}>
                        <p><strong>Computed SHA-256:</strong></p>
                        <code style={{ background: "#f4f4f4", padding: "5px", display: "block", wordBreak: "break-all" }}>{hash}</code>
                        <br />
                        <button
            ...
            <section>
                ...
                <div style={{ display: "flex", gap: "10px", marginBottom: "1rem" }}>
                    ...
                    />
                    <button onClick={verifyDocument} disabled={loading} style={{ padding: "8px 16px" }}>
                        Verify
                    </button>
                </div>
                <p style={{ fontSize: "0.9rem", color: "#666" }}>Or upload a file above to auto-fill its hash.</p>
                    ...
                    <div style={{ marginTop: "1rem", padding: "1rem", border: "1px solid #ccc", borderRadius: "5px" }}>
                        ...
                        {verifyResult.exists ? (
                            <>
                                ...
                                <p><strong>Timestamp:</strong> {new Date(verifyResult.timestamp * 1000).toLocaleString()}</p>
                            </>
                        ) : (
